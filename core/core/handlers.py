...
class BaseHandler(ABC):
    ...
    def handle(self, query: str, detections: List[Dict],
               ocr_results: List[Dict], **kwargs) -> List[Dict]:
        ...

    def process(self, query: str, detections: List[Dict],
                ocr_results: List[Dict], **kwargs) -> Optional[List[Dict]]:
        if self.can_handle(query):
            return self.handle(query, detections, ocr_results, **kwargs)
        elif self._next:
            return self._next.process(query, detections, ocr_results, **kwargs)
        return None
...
class PhoneUsageHandler(BaseHandler):
    ...
    def handle(self, query, detections, ocr_results, **kwargs):
        evidences = []
